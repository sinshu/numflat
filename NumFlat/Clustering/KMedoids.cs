...
namespace NumFlat.Clustering
{
    ...
    {
        private readonly Distance<T, T> distance;
        private readonly Medoid[] medoids;

        internal KMedoids(Distance<T, T> distance, Medoid[] medoids)
        {
        ...
        public (KMedoids<T> Model, double Error) UpdateSwap(IReadOnlyList<T> items)
        {
            ...
            {
                foreach (var mi in medoids)
                {
                    var tmp = distance(mi.Item, xo.Item);
                    if (tmp < dNearest[xo.Index])
                    {
                        dSecond[xo.Index] = dNearest[xo.Index];
                        dNearest[xo.Index] = tmp;
                        nearest[xo.Index] = mi.Index;
                    }
            ...
            while (true)
            {
                ...
                foreach (var mi in medoids)
                {
                    // each non-medoid
                    // foreach xc != { m1, ..., mk } do
                    foreach (var xc in KMedoids.EnumerateItemsExcept(items, medoids))
                    {
                        // dTD <- 0;
                        var dTd = 0.0;

                        // foreach xo != { m1, ..., mk } \ mi do
                        foreach (var xo in KMedoids.EnumerateItemsExceptBut(items, medoids, mi.Index))
                        {
                            // compute loss change
                            // dTD <- dTD + d(xo, mi, xc);
                        }
    ...
    public sealed class KMedoids
    {
        ...
        public static (KMedoids<T> Model, double Error) GetInitialGuessBuild<T>(IReadOnlyList<T> items, Distance<T, T> distance, int clusterCount)
        {
            // (TD, m1) <- (oo, null);
            ...
            {
                dNearest[xo.Index] = distance(items[m0], xo.Item);
            }
            ...
            for (var i = 0; i < clusterCount - 1; i++)
            {
                ...
                foreach (var xc in EnumerateItemsExcept(items, ms))
                {
                    ...
                    foreach (var xo in EnumerateItemsExcept(items, ms, xc.Index))
                    {
                        ...
                        var delta = distance(xo.Item, xc.Item) - dNearest[xo.Index];
                ...
                foreach (var xo in EnumerateItemsExcept(items, ms))
                {
                    ...
                    dNearest[xo.Index] = Math.Min(tmp, distance(xo.Item, items[mLast]));
                }
            }
        ...
        internal static IEnumerable<(int Index, T Item)> EnumerateItems<T>(IReadOnlyList<T> items)
        {
            var count = 0;
        ...
        internal static IEnumerable<(int Index, T Item)> EnumerateItemsExcept<T>(IReadOnlyList<T> items, int excludeIndex)
        {
        ...
        internal static IEnumerable<(int Index, T Item)> EnumerateItemsExcept<T>(IReadOnlyList<T> items, IReadOnlyList<int> excludeIndices)
        {
            foreach (var tuple in EnumerateItems(items))
            {
        ...
        internal static IEnumerable<(int Index, T Item)> EnumerateItemsExcept<T>(IReadOnlyList<T> items, IReadOnlyList<int> excludeIndices, int anotherExcludeIndex)
        {
        ...
        internal static IEnumerable<(int Index, T Item)> EnumerateItemsExcept<T>(IReadOnlyList<T> items, IReadOnlyList<KMedoids<T>.Medoid> medoids)
        {
            foreach (var tuple in EnumerateItems(items))
            {
                if (medoids.All(medoid => medoid.Index != tuple.Index))
                {
                    yield return tuple;
                }
            }
        ...
        internal static IEnumerable<(int Index, T Item)> EnumerateItemsExceptBut<T>(IReadOnlyList<T> items, IReadOnlyList<KMedoids<T>.Medoid> medoids, int addThisIndex)
        {
            foreach (var tuple in EnumerateItems(items))
            {
                if (medoids.All(medoid => medoid.Index != tuple.Index))
            ...
            yield return (addThisIndex, items[addThisIndex]);
        }
    }
}
